...
namespace Powercards.Core.Cards
{
    ...
    public class Remodel : CardBase, IActionCard
    {
        #region methods
        public void Play(TurnContext context)
        {
            if (!context.ActivePlayer.Hand.IsEmpty)
            {
                var trashCard = context.Game.Dialog.Select(context, context.ActivePlayer, context.ActivePlayer.Hand,
                    new CountValidator<ICard>(1), "Select a card to Remodel.").Single();
                ...
                var maxCostOfCardToGain = trashCard.GetCost(context, context.ActivePlayer) + 2;
            ...
            {
                context.Game.Log.LogMessage(context.ActivePlayer.Name + " did not have any cards to Remodel.");
            }
        }
        #endregion
    }
